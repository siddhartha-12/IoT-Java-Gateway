 ...
 * Class Declaration
 */
public class MqttUbidotConnector implements MqttCallback,Runnable{
	MqttClient client;
	 ...
	 */
	private Logger logger = Logger.getLogger(MqttUbidotConnector.class.getName());
	/***
	 * Class constructor
	 * @throws MqttException
	 */
	public MqttUbidotConnector() throws IOException, MqttException {
		super();
		ConfigUtil cu = new ConfigUtil();
		cu.loadConfig("config\\ConnectedDevicesConfig.props");
		this.host = cu.getValue("MQTTubidots.cloud", "host");
		this.port = cu.getValue("MQTTubidots.cloud", "port");
		this.topic = "/v1.6/devices/smartplant/actuatorcommand/lv";
		this.protocol ="tcp";
		this.qos = 0;
		this.mco = new MqttConnectOptions();
		this.mco.setUserName(cu.getValue("MQTTubidots.cloud", "apiKey"));
	 ...
	 * Constructor
	 */
	public MqttUbidotConnector(String host,String token,String FilePath) throws Exception
	 {
		this.protocol ="ssl";
		this.qos = 0;
		this.mco = new MqttConnectOptions();
	...
	public boolean ClientConnection()
	{
		try {
			...
			this.client = new MqttClient(ClientAddress,	MqttClient.generateClientId(),new MemoryPersistence());			
	...
	public void messageArrived(String topic, MqttMessage message) throws Exception {
		logger.info("Message received from Ubidots-> " + message.toString());
		if(message.toString().equals("1"))
		{
			this.mpb.setTopic("project/constraint/actuator");
